...
include_once "config.php";
 ...
 // selecteer de rij van de opgeven kroegcode uit de table bier
 function getbier($kroegcodes){
    // Connect database
    $conn = connectDb();
    ...
    $query = $conn->prepare($sql);
    $query->execute([':kroegcode'=>$kroegcodes]);
    $result = $query->fetch();
...
function printcrudkroeg($result){
    ...
    foreach ($result as $row) {
        ...
        // Wijzig knopje
        $table .= "<td>
            <form method='post' action='update_bier.php?kroegcode=$row[kroegcode]' >       
                <button>Wzg</button>	 
            </form></td>";

        // Delete knopje
        $table .= "<td>
            <form method='post' action='delete_bier.php?kroegcode=$row[kroegcode]' >       
...
function insertbier($post){
    ...
    // Maak een query 
    $sql = "
        INSERT INTO " . CRUD_TABLE . " (naam, stijl, kroegcode)
        VALUES (:naam, :stijl, :kroegcode) 
    ";
